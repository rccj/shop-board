...
interface Props {
  ...
  categories: Category[]
  onSave: (payload: Omit<Product, 'id' | 'createdAt' | 'updatedAt'>) => Promise<void>
}

export function ProductAddModal({ open, onOpenChange, categories, onSave }: Props) {
  ...
  const {
    ...
    defaultValues: { status: 'active', stock: 0, lowStockThreshold: 10 },
  })
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    const seed = values.name.toLowerCase().replace(/\s+/g, '')
    const image = imageValue || `https://picsum.photos/seed/${seed}/400/400`
    try {
      await onSave({
        name: values.name,
      ...
      toast.success('商品已新增', { description: values.name })
      reset()
      setImageValue('')
      setImageMode('url')
      onOpenChange(false)
      ...
      <DialogContent className="max-w-lg flex flex-col overflow-hidden">
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-3">
                ...
                <div className="flex-1 space-y-2">
                  ...
                  {imageMode === 'url' ? (
                    <Input placeholder="https://..." value={imageMode === 'url' && !imageValue.startsWith('data:') ? imageValue : ''} onChange={e => setImageValue(e.target.value)} />
                  ) : (
                    <>
                  ...
                  <p className="text-xs text-muted-foreground">不填則自動產生示意圖</p>
                </div>
              </div>
            </div>
          ...
          <DialogFooter className="pt-2">
            ...
            <Button type="submit" disabled={isSubmitting}>{isSubmitting ? '新增中…' : '新增商品'}</Button>
          </DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
