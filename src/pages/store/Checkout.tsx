                            ...
                            {cartDiscountItems.map(({ product, item, detail }) => (
                              <div key={`cd-${product.id}`} className="flex justify-between text-[10px] text-muted-foreground">
                                <span className="truncate mr-2">
                                  ...
                                  <span className="text-green-600 ml-1">({detail!.discountType === 'full_amount' ? '滿額9折' : detail!.discountType === 'second_item_half' ? '第二件半價' : `分類${Number.isInteger(detail!.discountRate * 10) ? detail!.discountRate * 10 : Math.round(detail!.discountRate * 100)}折`})</span>
                                </span>
